...
contract MultiFeeDistribution is IMultiFeeDistribution, Ownable {
    ...
    uint256 public constant rewardsDuration = 86400; //86400 * 7; //60;
    uint256 public constant rewardLookback = 86400; //30; //86400;
    
    // Duration of lock/earned penalty period
    uint256 public constant lockDuration = rewardsDuration * 7; // rewardsDuration * 4;
    ...
    function claimableRewards(address account) external view returns (RewardData[] memory claimRewards) {
        claimRewards = new RewardData[](rewardTokens.length);
        for (uint256 i = 0; i < claimRewards.length; i++) {
            ...
            claimRewards[i].token = rewardTokens[i];
            claimRewards[i].amount = _earned(account, claimRewards[i].token, balance, _rewardPerToken(rewardTokens[i], supply)).div(1e12);
        }
        return claimRewards;
    }
